function  sep_bar_chart(varargin)
...
p.addParameter('measures',{'P25Cc_N33Cc', 'N33Cc_P45Cc'}, @iscell);
...
if strcmp(inputs.file, 'none')		% no file specified
	...
	[fname, pathname] = uigetfile('*.xlsx', 'Pick sep_data_format_pre_post_diff.xlsx file');
...
for i_cnt = 1:length(inv_list)
	...
	for m_cnt = 1:length(inputs.measures)
		...
		% data for each bar
		hs_pst1 = table2array(tbl(tbl.measure==measure & tbl.arm==i_str & tbl.SessType==sess_list{1}, 'post1_minus_pre'));
		hs_pst2 = table2array(tbl(tbl.measure==measure & tbl.arm==i_str & tbl.SessType==sess_list{1}, 'post2_minus_pre'));
		ha_pst1 = table2array(tbl(tbl.measure==measure & tbl.arm==i_str & tbl.SessType==sess_list{2}, 'post1_minus_pre'));
		ha_pst2 = table2array(tbl(tbl.measure==measure & tbl.arm==i_str & tbl.SessType==sess_list{2}, 'post2_minus_pre'));
		l_pst1 = table2array(tbl(tbl.measure==measure & tbl.arm==i_str & tbl.SessType==sess_list{3}, 'post1_minus_pre'));
		l_pst2 = table2array(tbl(tbl.measure==measure & tbl.arm==i_str & tbl.SessType==sess_list{3}, 'post2_minus_pre'));
		...
		hold on
		add_error_bars(hb, mean_bar_values, std_values, axes_linewidth)
		...
		title(strrep(strrep(measure, '_', '-'), 'Cc', ''), 'FontSize', title_font_size)
		legend('post1','post2')
	end % measure
end % inv or un
...
function add_error_bars(hb, mean_bar_values, std_values, axes_linewidth)
...
for ib = 1:numel(hb)
      ...
      set(herr, 'LineWidth', axes_linewidth)
end
