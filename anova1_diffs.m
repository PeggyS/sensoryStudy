function anova1_diffs(varargin)
% anova1_DIFFS - anova1 test between  difference variables sham(Hs) vs L vs Ha
%
			...
			for ss = 1:length(perif_list),
				...
				if cnt > 0
					...
					if size(data_mat,1) > 1,
						[p,stat_tbl,stats] = anova1(data_mat, ...
...
% save
[filename, pathname] = uiputfile('*.txt', 'Save output stats table as', 'anova1_stats_table.txt');
