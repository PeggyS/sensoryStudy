function finalDS = merge2excels(startDir)
% merge2excels - read in 2 excel files and reformat into a single spreadsheet
% 
% The user will be asked to identify 2 excel files via open dialog boxes.
...
% Author: Peggy Skelly
% 2013-09-11: created (2-1/2 hrs)

% request file 1
[fname1, pathname1] = uigetfile('*.xlsx', 'Pick excel file #1');
if isequal(fname1,0) || isequal(pathname1,0)
	disp('User canceled. Exitting')
	return
end
% request file 2
[fname2, pathname2] = uigetfile('*.xlsx', 'Pick excel file #2');
...
ds1 = dataset('xlsfile', fullfile(pathname1,fname1));
ds2 = dataset('xlsfile', fullfile(pathname2,fname2));
...
% request save name
[fnameSave, pathnameSave] = uiputfile('*.xlsx', 'Save merged file as ...');
...
if isequal(fnameSave,0) || isequal(pathnameSave,0)
   disp('User pressed cancel. Nothing will be saved.')
	...
	export(newDS, 'XLSfile', fullfile(pathnameSave, fnameSave));
end
