function out_tbl = table_format_median(varargin)
%TABLE_FORMAT - format data into a table with cols for pre and delta Post1 and delta post2
%
% Use data from Combined_data_Sens_rTMS_20180316.csv. Create a table with
% columns: measure, pre, sham_delta_post1, sham_delta_post2, low_delta_post1, low_delta_post2,
%			hi_delta_post1, hi_delta_post2
%
%		reporting  median and iqr, and uninvolved or uninvolved side
%
...
tbl = tbl(:,[{'Subj', 'SessType', 'arm', 'pre_post'}, inputs.measures]);
...
% 	measure = inputs.measures{m_cnt};
	...
	pre_median = median(pre_data, 'omitnan');
	pre_iqr   = iqr(pre_data);
	pre_cnt  = sum(~isnan(pre_data));
	
	pre_median_iqr = format_with_parens(pre_median, pre_iqr);
	
	out_tbl.pre = pre_median_iqr';
...
for sess_cnt = 1:length(sess_list)
	...
	for post_cnt = 1:length(post_list)
		...
		pst_median = median(pst_data, 'omitnan');
		pst_iqr   = iqr(pst_data);
		pst_cnt  = sum(~isnan(pst_data));
		
		pst_median_iqr = format_with_parens(pst_median, pst_iqr);
		
		out_tbl.([sess '_' pst]) = pst_median_iqr';
		out_tbl.([sess '_' pst '_n']) = pst_cnt';
		
	end
end

writetable(out_tbl, ['median_table_' arm '.csv'])
